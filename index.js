...
let markets = ["STARBITS", "SIM", "BEER", "HUSTLER", "LEO", "LIST", "ECU", "WEED"]
let username = "deathbing"
...
async function getBalances() {
  ...
  for (let i in balances) {
    if (parseFloat(balances[i].balance) > 0) {
      nonZeroBalances[balances[i].symbol] = parseFloat(balances[i].balance) / 7.69
    }
  }
...
function placeOrders(balances, prices) {
  ...
  for (let i in balances) {
    if (prices[i]) {
      orderJSON.push({ "contractName": "market", "contractAction": `sell`, "contractPayload": { "symbol": i, "quantity": `${parseInt(balances[i])}`, "price": `${(parseFloat(prices[i]) - 0.00000001).toFixed(8)}` } })
    }
  }
